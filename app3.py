...
class GoldApp:
    ...
    def _create_widgets(self):
        ...
        input_frame.pack(fill=X, pady=(0, 10), expand=YES)
        ...
        ttk.Label(input_frame, text="تاریخ شروع (مثال: 1403-01-01):").grid(row=1, column=0, padx=5, pady=10, sticky="w")
        self.start_date_entry = ttk.Entry(input_frame, bootstyle="primary")
        self.start_date_entry.grid(row=1, column=1, columnspan=2, padx=5, pady=10, sticky="ew")
        self.start_date_entry.insert(0, jdatetime.date(1403, 1, 1).isoformat())

        ttk.Label(input_frame, text="تاریخ پایان (مثال: 1404-05-31):").grid(row=2, column=0, padx=5, pady=10, sticky="w")
        self.end_date_entry = ttk.Entry(input_frame, bootstyle="primary")
        self.end_date_entry.grid(row=2, column=1, columnspan=2, padx=5, pady=10, sticky="ew")
        self.end_date_entry.insert(0, jdatetime.date.today().isoformat())

        ttk.Label(input_frame, text="مسیر ذخیره فایل:").grid(row=3, column=0, padx=5, pady=10, sticky="w")
        self.output_path_entry = ttk.Entry(input_frame, bootstyle="primary")
        self.output_path_entry.grid(row=3, column=1, padx=5, pady=10, sticky="ew")
    ...
    def browse_output_path(self):
        filepath = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx"), ("All files", "*.*")], initialfile=self.output_path_entry.get())
        if filepath:
    ...
    def start_fetching(self):
        ...
        if not re.match(r"^\d{4}-\d{2}-\d{2}$", start_date_str) or not re.match(r"^\d{4}-\d{2}-\d{2}$", end_date_str):
            messagebox.showerror("خطای فرمت", "لطفا تاریخ را با فرمت صحیح YYYY-MM-DD وارد کنید.", parent=self.master)
            return
